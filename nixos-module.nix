    ...
    options = {
      ...
      interfaces = lib.mkOption {
        ...
        description = ''
          ...
          When useNetlinkd is set, these are also passed to the shared
...
in
{
  ...
  config =
    let
      ...
      instanceData = lib.mapAttrs (
        ...
        let
          ...
          renderConfig = pkgs.writeShellScript "gremlind-${name}-render-config" ''
            ...
            ''}
            ${lib.optionalString (cfg.interfaces != { }) ''
              echo 'interfaces:' >> "$out"
              ${lib.concatMapStringsSep "\n" (id: ''
      ...
      # Operator-pinned interface names from every instance that hands
      # provisioning to the shared broker, so netlinkd's allow-list
      # covers them regardless of which instance owns each name.
      allowedIfaces = lib.concatLists (
        lib.mapAttrsToList (
          _: cfg: lib.optionals cfg.useNetlinkd (lib.attrValues cfg.interfaces)
        ) config.services.gremlind
      );
    in
    {
              ...
              in
              {
                ...
                serviceConfig = {
                  ExecStart = "${lib.getExe ncfg.package} netlinkd -s ${ncfg.socket} -mode 0660 -group ${ncfg.group}"
                    ...
                    + lib.concatMapStrings (n: " -iface ${n}") allowedIfaces;
                  Restart = "always";
                  RestartSec = 3;
                  UMask = "0077";
